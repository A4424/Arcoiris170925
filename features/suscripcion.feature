...
Feature: Gestion del perfil y suscripcion en el sitio PuntosPremiumPlus
  ...
  @login @perfil
  Scenario: Acceder a la informacion del perfil de usuario
...
@login @suscripcion
Scenario: Activar la suscripcion a promociones
    Given Se tiene una sesion iniciada
    When Se hace clic en el menu hamburguesa
    And Se hace clic en el enlace "PERFIL"
    Then Se muestra el contenido del perfil de usuario
    When Se actualiza la suscripcion a promociones a "activada"
    And Se hace click en el boton aceptar
    Then Se verifica que la suscripcion esté activada

@login @suscripcion @desactivar
Scenario: Desactivar la suscripcion a promociones
    Given Se tiene una sesion iniciada
    When Se hace clic en el menu hamburguesa
    And Se hace clic en el enlace "PERFIL"
    Then Se muestra el contenido del perfil de usuario
    When Se actualiza la suscripcion a promociones a "desactivada"
    And Se hace click en el boton aceptar
